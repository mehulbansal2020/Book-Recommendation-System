...
books <- fread('D:/Sem 6/DV/books.csv')
ratings <- fread('D:/Sem 6/DV/ratings.csv')
book_tags <- fread('D:/Sem 6/DV/book_tags.csv')
tags <- fread('D:/Sem 6/DV/tags.csv')
```
...
### Let's start the Exploration with full josh
...
For this step we select users that have in common that they rated the same books. To make it easier let's select one example user "David" (user_id: 13812). First we select users that rated at least one book that David also rated. In total there are 40 users who have at least one book in common. 

```{r}
current_user <- "13812"
#print(as.data.frame(ratingmat))
...
I do this for 2 users (user_ids: 758 and 13812) for illustration.

```{r}
user1 <- data.frame(item=colnames(ratingmat),rating=ratingmat[current_user,]) %>% filter(!is.na(rating))
user2 <- data.frame(item=colnames(ratingmat),rating=ratingmat["758",]) %>% filter(!is.na(rating))
tmp<-merge(user1, user2, by="item")
print(tmp)
#cor(tmp$rating.x, tmp$rating.y, use="pairwise.complete.obs")

#user2 <- data.frame(item = colnames(ratingmat), rating = ratingmat["13812", ]) %>% filter(!is.na(rating))
#tmp <- merge(user1, user2, by="item")
#tmp
#cor(tmp$rating.x, tmp$rating.y, use="pairwise.complete.obs")
```
